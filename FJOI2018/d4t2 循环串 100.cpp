...
解法：根据抽屉原理，至少有一个字母出现超过n/m次
把这个字母单独拉出来，价值为n^2/(m^2)，所以我们只要处理长度<m^2的循环节
如果循环节中存在字母出现>=m次，设循环节出现次数为k
那把这个字母拉出来，价值为m^2*k^2，而原来价值至多为(m^2-1)k^2
所以我们只要枚举每个字母出现<m次的循环节 
*/ 
#include <bits/stdc++.h>
using namespace std;
